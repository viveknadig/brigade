...
persist_path() {
  ...
  for rc in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.profile"; do
    [ -f "$rc" ] || continue
    if ! grep -qF "${RUNTIME_DIR}/bin" "$rc" 2>/dev/null; then
...
main() {
  ...
  npm i -g "$BRIGADE_PKG"
  printf '\n\033[1;32m✓ Brigade installed.\033[0m  Run:  \033[1mbrigade onboard\033[0m\n'
  if [ "$NODE_FRESHLY_INSTALLED" -eq 1 ]; then
    printf '   Node was just installed — open a new terminal first so it is on your PATH.\n'
  fi
}
