...
export const write = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.expectedSha256 !== undefined &&
            ...
            throw new Error(`OCC conflict: expected sha256=${args.expectedSha256} but on-disk is ${existing.contentSha256}`);
        }
        const payload = {
...
function reconstructConfig(row) {
    const out = {};
    for (const k of [
    ...
    if (extra && typeof extra === "object") {
        for (const [k, v2] of Object.entries(extra))
            if (out[k] === undefined)
                out[k] = v2;
    }
    return out;
...
async function captureBackup(ctx, instanceId, priorRow) {
    const existing = await ctx.db
        .query("brigadeConfigBackups")
        .withIndex("by_instance_slot", (q) => q.eq("instanceId", instanceId))
        .collect();
    for (const b of existing) {
        if (b.slot >= BACKUP_COUNT - 1)
            await ctx.db.delete(b._id);
        else
            await ctx.db.patch(b._id, { slot: b.slot + 1 });
    }
    await ctx.db.insert("brigadeConfigBackups", {
        instanceId,
        slot: 0,
        contentSha256: priorRow.contentSha256 ?? "",
        payload: JSON.stringify(reconstructConfig(priorRow)),
        bytes: priorRow.bytes ?? 0,
...
export const getBackup = query({
    ...
    handler: async (ctx, args) => {
        const row = await ctx.db
            .query("brigadeConfigBackups")
            .withIndex("by_instance_slot", (q) => q.eq("instanceId", args.instanceId).eq("slot", args.slot))
            .first();
