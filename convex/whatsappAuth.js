...
export const loadAll = query({
    ...
    handler: async (ctx, args) => {
        const credsRow = await ctx.db
            ...
            .withIndex("by_owner_account", (q) => q.eq("ownerId", args.ownerId).eq("accountId", args.accountId))
            .first();
...
export const writeKeys = mutation({
    ...
    handler: async (ctx, args) => {
        for (const entry of args.entries) {
            const existing = await ctx.db
                .query("whatsappAuthKeys")
                .withIndex("by_owner_account_type_id", (q) => q
...
export const clearAccount = mutation({
    ...
    handler: async (ctx, args) => {
        const credsRow = await ctx.db
            ...
            .first();
        if (credsRow)
            await ctx.db.delete(credsRow._id);
        const keyRows = await ctx.db
            .query("whatsappAuthKeys")
            .withIndex("by_owner_account", (q) => q.eq("ownerId", args.ownerId).eq("accountId", args.accountId))
            .collect();
...
//# sourceMappingURL=whatsappAuth.js.map