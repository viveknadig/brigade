...
export const getByChildSessionKey = query({
    ...
    handler: async (ctx, args) => {
        return ctx.db
            .query("subagentRuns")
            .withIndex("by_childSessionKey_active", (q) => q.eq("ownerId", args.ownerId).eq("childSessionKey", args.childSessionKey))
            .order("desc")
            .first();
    },
});
export const listByRequester = query({
    ...
    handler: async (ctx, args) => {
        return ctx.db
            .query("subagentRuns")
            .withIndex("by_requester_createdAt", (q) => q.eq("ownerId", args.ownerId).eq("requesterSessionKey", args.requesterSessionKey))
            .collect();
    },
});
export const put = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (existing) {
            ...
            await ctx.db.insert("subagentRuns", payload);
        }
    },
});
export const markCompleted = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        await ctx.db.patch(existing._id, {
            ...
            ...(args.error !== undefined ? { outcome: { ...args.outcome, error: args.error } } : {}),
