...
const Source = v.union(v.literal("bundled"), v.literal("config"), v.literal("managed"), v.literal("personal"), v.literal("project"), v.literal("workspace"));
export const list = query({
    ...
    handler: async (ctx, args) => {
        if (args.source !== undefined && args.agentId !== undefined) {
            ...
            return ctx.db
                .query("skills")
                .withIndex("by_owner_scope_name", (q) => q.eq("ownerId", args.ownerId).eq("source", source).eq("agentId", agentId))
                .collect();
        }
