...
export const listJobs = query({
    ...
    handler: async (ctx, args) => {
        if (args.enabledOnly) {
            return ctx.db
                .query("cronJobs")
                .withIndex("by_owner_enabled_next", (q) => q.eq("ownerUserId", args.ownerUserId).eq("enabled", true))
                .collect();
        }
        return ctx.db
            .query("cronJobs")
...
export const getJob = query({
    ...
    handler: async (ctx, args) => {
        return ctx.db
            .query("cronJobs")
            .withIndex("by_owner_job", (q) => q.eq("ownerUserId", args.ownerUserId).eq("jobId", args.jobId))
            .first();
    },
});
...
export const patchJob = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            ...
            .first();
        if (!existing)
            throw new Error(`cron: job ${args.jobId} not found`);
        const patchObj = {
            ...args.patch,
            updatedAtMs: Date.now(),
        };
        if (args.unset) {
            for (const col of args.unset)
                patchObj[col] = undefined;
        }
...
export const markRunning = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!job)
            return false;
        if (job.stateRunningAtMs && job.stateRunningAtMs > 0)
            return false;
...
export const listRunLog = query({
    ...
    handler: async (ctx, args) => {
        ...
        return ctx.db
            .query("cronRuns")
            .withIndex("by_owner_job_ts", (q) => q.eq("ownerUserId", args.ownerUserId).eq("jobId", args.jobId))
            .order("desc")
            .take(limit);
    },
});
