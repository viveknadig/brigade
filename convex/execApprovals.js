...
export const list = query({
    ...
    handler: async (ctx, args) => {
        return ctx.db
            .query("execApprovals")
            .withIndex("by_owner_agent_kind", (q) => q.eq("ownerId", args.ownerId).eq("agentId", args.agentId))
            .collect();
    },
});
export const insert = mutation({
    ...
    handler: async (ctx, args) => {
        const existing = await ctx.db
            .query("execApprovals")
            .withIndex("by_owner_agent_value", (q) => q
...
//# sourceMappingURL=execApprovals.js.map