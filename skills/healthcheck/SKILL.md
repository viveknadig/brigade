...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Brigade can access sensitive data.
...
Ongoing monitoring is recommended. Use the Brigade cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Before creating, `brigade cron list` and match on exact `name`. If found, `brigade cron edit <id> ...`.
If not found, `brigade cron add --name <name> ...`.
