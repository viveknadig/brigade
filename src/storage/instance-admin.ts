...
// NOTE: `convex/_generated/api.js` is loaded LAZILY (inside the async functions
// below), never as a top-level import. It exists only in a convex-mode dev
// checkout (generated by `convex codegen`) and is intentionally NOT shipped in
// the published npm package — the `convex/` dir is dev/build-only. A static
// import here would crash the DEFAULT (filesystem) onboarding wizard, which
// imports this module just to offer the convex-mode option. See the `files`
