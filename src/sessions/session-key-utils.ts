/**
 * Session-key parsing + classification primitives.
 *
 * Brand-scrubbed lift of the upstream reference codebase's
 * `src/sessions/session-key-utils.ts`. Every parser preserves the canonical
 * `agent:<agentId>:<rest>` format and the `:thread:<id>` suffix convention
 * verbatim — these shapes are the source of truth for routing, lane
 * resolution, and cross-session tools.
 *
 * What's lifted:
 *   - `parseAgentSessionKey`        — canonical `agent:<id>:<rest>` parser
 *   - `isCronRunSessionKey`         — matches `rest = cron:<job>:run:<runId>`
 *   - `isCronSessionKey`            — matches `rest = cron:*`
 *   - `isSubagentSessionKey`        — matches `subagent:*` at any depth
 *   - `getSubagentDepth`            — counts `:subagent:` segments
 *   - `parseThreadSessionSuffix`    — splits `<key>:thread:<id>` shape
 *   - `parseRawSessionConversationRef` — extract channel + kind + raw id
 *   - `resolveThreadParentSessionKey` — base key of a thread-suffixed session
 *
 * Intentionally NOT lifted: `isAcpSessionKey`. Brigade is subagent-only;
 * the ACP harness runtime is dropped from the lift per R2 Leak #10 of the
 * locked design.
 *
 * All entry points return `false` / `null` / `undefined` for invalid
 * input — never throw. Lower-cased internally for stable comparisons; the
 * returned `agentId` and `rest` are already normalised.
 */
 ...
 * Returned values are normalised to lowercase for stable comparisons / routing.
 */
