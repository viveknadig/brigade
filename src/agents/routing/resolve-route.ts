/**
 * 8-tier agent route resolver.
 *
 * Brand-scrubbed lift of the upstream reference codebase's
 * `src/routing/resolve-route.ts`. Every algorithm — the 8-tier waterfall
 * cascade, the per-config WeakMap caches (agent lookup, evaluated
 * bindings, resolved routes), the binding-index builder, the
 * peer/account/guild/team scope matcher — is preserved verbatim. Only
 * adjustments:
 *
 *   - upstream `Config` type → `BrigadeConfig` (Brigade's type)
 *   - `buildAgentMainSessionKey` → `buildBrigadeMainSessionKey` (R2 Leak #2)
 *   - `shouldLogVerbose()` reads `BRIGADE_LOG_ROUTING` env var (Brigade-native verbose flag)
 *   - `logDebug(msg)` routes through `createSubsystemLogger("routing").info(msg)`
 *
 * Cache architecture (preserved verbatim):
 *   - `agentLookupCacheByCfg` — keyed on `cfg.agents` reference identity;
 *     maps normalized agent ids back to their canonical casing
 *   - `evaluatedBindingsCacheByCfg` — keyed on `cfg.bindings` reference;
 *     stores per-channel + per-(channel, account) pre-normalized binding
 *     records + index structures (peer / wildcard / guild+roles / guild /
 *     team / account / channel lookup maps)
 *   - `resolvedRouteCacheByCfg` — keyed on (cfg.bindings, cfg.agents,
 *     cfg.session) triple; stores fully resolved routes by
 *     channel+account+peer+parentPeer+guildId+teamId+roleIds+dmScope key
 *
 * The 8 tiers, in waterfall order (first match wins):
 *   1. binding.peer           — exact peer match (peer kind + id)
 *   2. binding.peer.parent    — thread inheritance from parent peer
 *   3. binding.peer.wildcard  — peer kind matches, id is `"*"`
 *   4. binding.guild+roles    — guild + member role allowlist (Discord)
 *   5. binding.guild          — guild only (no roles constraint)
 *   6. binding.team           — Slack team
 *   7. binding.account        — explicit account constraint
 *   8. binding.channel        — channel-wildcard (`accountId === "*"`)
...
import type { BrigadeConfig, BindingEntry } from "../../config/types.js";
...
/** Verbose-routing toggle. Set `BRIGADE_LOG_ROUTING=1` to emit per-tier
 *  evaluation traces for every `resolveAgentRoute` call. Cached config
 *  paths bypass logging — only the cold path runs through here. */
function shouldLogVerbose(): boolean {
	return process.env.BRIGADE_LOG_ROUTING === "1";
}
...
function listAgents(cfg: BrigadeConfig): Array<{ id?: string }> {
	const agents = (cfg.agents as { list?: unknown } | undefined)?.list;
	return Array.isArray(agents) ? (agents as Array<{ id?: string }>) : [];
}

type AgentLookupCache = {
	agentsRef: BrigadeConfig["agents"] | undefined;
	byNormalizedId: Map<string, string>;
	fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<BrigadeConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: BrigadeConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: BrigadeConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
	bindingsRef: BrigadeConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<BrigadeConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
	BrigadeConfig,
	{
		bindingsRef: BrigadeConfig["bindings"];
		agentsRef: BrigadeConfig["agents"];
		sessionRef: BrigadeConfig["session"];
...
function resolveRouteCacheForConfig(cfg: BrigadeConfig): Map<string, ResolvedAgentRoute> {
...
function formatRoleIdsCacheKey(roleIds: string[]): string {
	...
	// `Array.prototype.toSorted` is ES2023; Brigade's tsconfig targets earlier.
	// Use the clone-then-sort idiom which is equivalent for our string array.
