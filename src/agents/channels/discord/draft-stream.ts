...
/** Injected Discord transport — exactly the two calls the stream needs. */
export interface DraftStreamTransport {
	/** Post a new message; returns its id. */
	postMessage(text: string, opts: { threadId?: string }): Promise<{ id: string }>;
	/** Edit a previously-posted message's text. */
	editMessage(id: string, text: string): Promise<void>;
}

export interface CreateDraftStreamParams {
	transport: DraftStreamTransport;
	/** Thread id for the FIRST (placeholder) post. */
	threadId?: string;
	/** Minimum ms between edits. Clamped to ≥ 250. Default 1000. */
	throttleMs?: number;
	/** Per-message char cap. Clamped to ≤ 2000. Default 2000. */
	maxChars?: number;
	/** Convert a markdown chunk to Discord markup (or plain on failure). */
