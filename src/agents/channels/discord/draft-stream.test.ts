...
import { createDraftStream, splitAtBoundary, DISCORD_STREAM_MAX_CHARS } from "./draft-stream.js";

/** A fake transport recording every post/edit so the state machine is observable. */
...
describe("createDraftStream", () => {
	...
	it("applies a render hook (markdown→discord)", async () => {
		const { posts, transport } = makeFakeTransport();
	...
	it("forwards threadId on the first post", async () => {
		...
		assert.equal(posts[0]?.threadId, "42");
	});

	it("caps maxChars at Discord's hard limit", async () => {
		const { transport } = makeFakeTransport();
		const stream = createDraftStream({ transport, maxChars: 999_999 });
		const huge = "z".repeat(DISCORD_STREAM_MAX_CHARS + 100);
