/**
 * Telegram reasoning-lane split (OPTIONAL, default OFF).
 *
 ...
 * Telegram an operator can OPT IN (config `channels.telegram.surfaceReasoning:
 ...
 * calls this and behavior is byte-identical to today (strip + send answer).
 * When ON, the pipeline sends `reasoningText` first (a `🧠 Reasoning:` block)
 * then the normal sanitized answer.
 *
...
export interface TelegramReasoningSplit {
 ...
 */
export function splitTelegramReasoning(raw: string): TelegramReasoningSplit {
