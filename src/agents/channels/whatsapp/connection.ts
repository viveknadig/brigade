 ...
 *
 * Brigade-native port of the proven OpenClaw connect loop: a multi-file auth
 * store on disk, QR-on-first-link, auto-reconnect on transient drops, and a
 * normalized text-message callback. Baileys is a heavy dependency, so it is
 * lazy-imported here — the gateway only pays for it when a WhatsApp channel
 * actually starts. Types are imported `type`-only (erased at build) so the
 * static import never pulls the runtime in.
 *
...
export interface WhatsAppConnection {
	...
	/** Send a text message to a chat JID. Waits for a live socket. */
...
function extractText(message: WAMessage["message"], normalize: (m: unknown) => unknown): string {
	...
	// Image/video with a caption — treat the caption as the text.
 ...
 * first socket is constructed (NOT once connected — the QR/open events arrive
 * via the callbacks). The returned handle owns the reconnect loop.
 */
		...
		s.ev.on("connection.update", (update: Partial<ConnectionState>) => {
			...
			if (connection === "close") {
				...
				if (status === loggedOutCode) {
					...
					args.onLoggedOut?.();
					return; // do not reconnect with dead creds
				}
				if (!closed) {
					args.log("WhatsApp connection dropped — reconnecting", { status });
					sock = buildSocket(); // transient drop → fresh socket, same creds
				}
			}
		});
	...
	return {
		...
		async close(): Promise<void> {
			...
			try {
				...
				// keeps the link, so just end the underlying socket.
				sock?.end?.(undefined);
			} catch {
				/* already torn down */
			}
